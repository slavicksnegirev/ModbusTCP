...
namespace Client
{
	public class ModbusTCP
	{
		...
		private byte[] _unitID = new byte[1];
        ...
        public ModbusTCP(ushort transactionID, ushort protocolID, /*ushort lenght, */byte uintID/*, byte functionCode, byte[] data*/)
		{
			...
			UnitID = BitConverter.GetBytes(uintID);
		...
		public void RequestToServer(byte functionCode, byte[] data)
		{
			bool flag = false;
		...
		public void ResponseFromServer(byte functionCode, byte[] buffer)
		{
			bool flag = false;
			this.FunctionCode = BitConverter.GetBytes(functionCode);
